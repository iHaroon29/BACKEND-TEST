...
module.exports={
    createNewAssignmentSubmission(assignmentSubmissionDetails){
        return new AssignmentSubmission(assignmentSubmissionDetails).save()
            .then((savedDetails)=>{
                return savedDetails;
            }).catch(e=>{
                throw DaoError("unable to create a new assignment submission",503,e)
            })
    },
    ...
    async getAllSubmittedAssignmentsOfClass(classroomId) {
        try{
            ...
            for(let i in classroom.enrolled_courses){
                allAssignments.push(await this.getSubmittedAssignmentOfACourse(i));
            }
    ...
    async getSubmittedAssignmentOfACourse(courseId) {
        try{
            const allAssignmentsInCourse=JSON.parse(JSON.stringify(await Assignment.find({"course_id":courseId})));
            for(let i in allAssignmentsInCourse){
                allAssignmentsInCourse[i]=await AssignmentSubmission.find({"assignment_id":allAssignmentsInCourse[i]._id});
                allAssignmentsInCourse[i]=JSON.parse(JSON.stringify(allAssignmentsInCourse[i]));
            }
    ...
    deleteAssignmentSubmission(assignmentSubmissionId){
        return  AssignmentSubmission.findByIdAndDelete(assignmentSubmissionId)
            .then((deletedAssignmentSubmission)=>{
                ...
                throw DaoError("unable to delete assignment submission",503,e)
            })
    }
};